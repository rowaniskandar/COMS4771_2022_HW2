...
num_features = size(data_train,2)-1;
...
data_train.agecat = ordinal(data_train.age,{'under 25','25-31','32-42', 'above 42'},...
    [],[quantile_age(1),quantile_age(2),quantile_age(3),quantile_age(4),quantile_age(5)]);
getlevels(data_train.agecat)
...
data_train.juvfelcat = ordinal(data_train.juv_fel_count,{'0','1','above 1'},...
...
data_train.juvmisdcat = ordinal(data_train.juv_misd_count,{'0','1','above 1'},...
...
data_train.juvothercat = ordinal(data_train.juv_other_count,{'0','1','above 1'},...
...
data_train.priorscat = ordinal(data_train.priors_count,{'0','1','2','3','above 3'},...
...
data_test.agecat = ordinal(data_test.age,{'under 25','25-31','32-42', 'above 42'},...
    [],[quantile_age(1),quantile_age(2),quantile_age(3),quantile_age(4),quantile_age(5)]);
getlevels(data_test.agecat)
...
data_test.juvfelcat = ordinal(data_test.juv_fel_count,{'0','1','above 1'},...
    [],[0,1,2,quantile_juvfel(5)]);
unique_juvfel = unique(data_test.juv_fel_count);
...
data_test.juvmisdcat = ordinal(data_test.juv_misd_count,{'0','1','above 1'},...
    [],[0,1,2,quantile_juvmisd(5)]);
...
data_test.juvothercat = ordinal(data_test.juv_other_count,{'0','1','above 1'},...
    [],[0,1,2,quantile_juvother(5)]);
...
data_test.priorscat = ordinal(data_test.priors_count,{'0','1','2','3','above 3'},...
    [],[0,1,2,3,4,quantile_priors(5)]);
...
%start of analysis
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
X = removevars(data_train,'two_year_recid');
X = table2array(X);
Y = table2array(data_train(:,1));
%summary statistics by potentially "sensitive" feature
data_sex = grpstats(data_train,"sex");
data_race = grpstats(data_train,"race");
data_recid = grpstats(data_train,"two_year_recid");
...
performance_knn_euclid=zeros(1,k_cv);
performance_knn_l1=zeros(1,k_cv);
performance_knn_linf=zeros(1,k_cv);
...
for i=1:k_cv
    ...
    performance_gaussian(i) = classifier_gaussian_mv_err(X_train, Y_train, x_test, num_class, num_features);
    %k-nn classifier
